...
class PDFGenerator:
    ...
    def clean_text_for_pdf(text: str) -> str:
        ...
        # Clean bullet points but preserve them
        ...
        # Final ASCII encoding but preserve line structure
        lines = text.split('\n')
        cleaned_lines = []
        for line in lines:
            ...
            for paragraph in paragraphs:
                ...
                for line in lines:
                    ...
                    if line.startswith('###'):
                        ...
                        header_text = safe_line.replace('#', '').strip()
                        try:
                            pdf.cell(usable_width, 10, header_text, ln=True)
                        except:
                            pdf.cell(usable_width, 10, "[Title]", ln=True)
                        pdf.set_font("Arial", "", 11)
                        pdf.ln(4)
    ...
    with gr.Blocks(theme=theme, title="TrainBot - AI Course Architect") as interface:
        ...
        def handle_course_generation(mode, title, modules, level, f_count, language, selected_files):
            ...
            if mode == "Micro Course":
                course_title = f"Micro-Course: {title}"
            elif mode == "Quiz":
                course_title = f"Quiz: {title}"
            elif mode == "Flashcards":
                course_title = f"Flashcards: {title}"
            else:  # Full Course
                course_title = title  # Keep original title
            
            content = core.generate_course_content(
                mode, course_title, modules, level, f_count, language, selected_files
            )
        ...
        generate_btn.click(
            handle_course_generation,
            inputs=[mode, title, modules, level, flashcard_count, language, kb_selector],
            outputs=[course_output, generated_content_state]  # Remove the extra gr.State()
        )
        ...
        export_course_btn.click(
            ...
            inputs=[generated_content_state, title, mode],  # Pass mode for dynamic naming
            outputs=[course_pdf]
        )
